...
from stat_utils import get_distrib_params, ent, ent_new, P_hat, E_hat_do_x_cA, E_true_do_x_cA
from graph import Node, Graph, CPD, CPDCollection

import pprint

from stat_utils import get_V_index
...
# %%
from env_setting import num_value_c2, num_value_c1, num_value_c0, num_value_x, domain_y
...
g.insert('C2', [], np.arange(num_value_c2), cat='CA')
g.insert('C1', [], np.arange(num_value_c1), cat='CA')
g.insert('C0', ['C2'], np.arange(num_value_c0), cat='CB')
g.insert('X', ['C1'], np.arange(num_value_x))
g.insert('Y', ['X', 'C1', 'C0'], np.array(domain_y))
...
# %%
from stat_utils import get_parent_values_constrained
...
def sample_true_prob():
    ...
    prob_dict["C2_[]"] = [0.7, 0.11, 0.09, 0.1] 

    prob_dict["C1_[]"] = [0.31, 0.28, 0.41] 

    prob_dict["C0_[0]"] = [3211/5239, 241/5239, 200/5239, 1587/5239]
    prob_dict["C0_[1]"] = [214/412, 51/412, 51/412, 96/412]
    prob_dict["C0_[2]"] = [601/672, 18/672, 18/672, 35/672]
    prob_dict["C0_[3]"] = [0.1, 0.1, 0.6, 0.2]

    ## x=0

    prob_dict["Y_[0 0 0]"] = [1-36/56, 36/56, 0]
    prob_dict["Y_[0 0 1]"] = [4/6, 2/6, 0]
    prob_dict["Y_[0 0 2]"] = [0, 1, 0]
    prob_dict["Y_[0 0 3]"] = [0.5, 0.5, 0]

    prob_dict["Y_[0 1 0]"] = [1-45/75, 45/75, 0]
    prob_dict["Y_[0 1 1]"] = [4/6, 2/6, 0]
    prob_dict["Y_[0 1 2]"] = [0, 1, 0]
    prob_dict["Y_[0 1 3]"] = [0.5, 0.5, 0]

    prob_dict["Y_[0 2 0]"] = [1-28/539, 0, 28/539]
    prob_dict["Y_[0 2 1]"] = [6/7, 0, 1/7]
    prob_dict["Y_[0 2 2]"] = [0.5, 0, 0.5]
    prob_dict["Y_[0 2 3]"] = [2/5, 0, 3/5]

    ## x=1

    prob_dict["Y_[1 0 0]"] = [1-16/29, 16/29, 0]
    prob_dict["Y_[1 0 1]"] = [1, 0, 0]
    prob_dict["Y_[1 0 2]"] = [0.5, 0.5, 0]
    prob_dict["Y_[1 0 3]"] = [0.5, 0.5, 0]

    prob_dict["Y_[1 1 0]"] = [1-12/27, 12/27, 0]
    prob_dict["Y_[1 1 1]"] = [1, 0, 0]
    prob_dict["Y_[1 1 2]"] = [0.5, 0.5, 0]
    prob_dict["Y_[1 1 3]"] = [0.5, 0.5, 0]

    prob_dict["Y_[1 2 0]"] = [1-7/13, 0, 7/13]
    prob_dict["Y_[1 2 1]"] = [3/4, 0, 1/4]
    prob_dict["Y_[1 2 2]"] = [0.5, 0, 0.5]
    prob_dict["Y_[1 2 3]"] = [0, 0, 1]

    ## x=2

    prob_dict["Y_[2 0 0]"] = [1-8/13, 8/13, 0]
    prob_dict["Y_[2 0 1]"] = [1, 0, 0]
    prob_dict["Y_[2 0 2]"] = [0.5, 0.5, 0]
    prob_dict["Y_[2 0 3]"] = [0, 1, 0]

    prob_dict["Y_[2 1 0]"] = [1-3/8, 3/8, 0]
    prob_dict["Y_[2 1 1]"] = [1, 0, 0]
    prob_dict["Y_[2 1 2]"] = [0.5, 0.5, 0]
    prob_dict["Y_[2 1 3]"] = [0.5, 0.5, 0]

    prob_dict["Y_[2 2 0]"] = [1, 0, 0]
    prob_dict["Y_[2 2 1]"] = [1, 0, 0]
    prob_dict["Y_[2 2 2]"] = [0.5, 0, 0.5]
    prob_dict["Y_[2 2 3]"] = [0.5, 0, 0.5]

    ## x=3

    prob_dict["Y_[3 0 0]"] = [1-10/16, 10/16, 0]
    prob_dict["Y_[3 0 1]"] = [1, 0, 0]
    prob_dict["Y_[3 0 2]"] = [0, 1, 0]
    prob_dict["Y_[3 0 3]"] = [0.5, 0.5, 0]

    prob_dict["Y_[3 1 0]"] = [1-3/5, 3/5, 0]
    prob_dict["Y_[3 1 1]"] = [0.5, 0.5, 0]
    prob_dict["Y_[3 1 2]"] = [0.5, 0.5, 0]
    prob_dict["Y_[3 1 3]"] = [0.5, 0.5, 0]

    prob_dict["Y_[3 2 0]"] = [0, 0, 1]
    prob_dict["Y_[3 2 1]"] = [0.5, 0, 0.5]
    prob_dict["Y_[3 2 2]"] = [0.5, 0, 0.5]
    prob_dict["Y_[3 2 3]"] = [1, 0, 0]

    ##

    prob_dict["X_[0]"] = [0.25]*4
    prob_dict["X_[1]"] = [0.25]*4
    prob_dict["X_[2]"] = [0.25]*4
...
for i in range(NUM_RUNS_PER_B*len(B_VALUES)):
    ...
    if MLFLOW_ENABLED:
        ...
        mlflow.log_artifact("env_setting.py")
