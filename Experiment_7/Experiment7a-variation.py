...
# %%
from env_setting import num_value_c1, num_value_c0, num_value_x, domain_y
...
def sample_true_prob():
    ...
    cutoff = np.random.randint(1, int(num_value_c1/2))
    prob_weights = [np.random.randint(1, 11) for _ in range(0, cutoff)]
    p1 = [_/sum(prob_weights)*0.2 for _ in prob_weights]
    prob_weights = [np.random.randint(1, 11) for _ in range(cutoff, num_value_c1)]
    p2 = [_/sum(prob_weights)*0.8 for _ in prob_weights]
    prob_dict["C1_[]"] = p1 + p2
    
    for c1 in range(num_value_c1):
        if c1<cutoff:
            prob_dict[f"C0_[{c1}]"] = [1] + [0]*(num_value_c0-1)
        else:
            idx_of_1 = (c1-cutoff)%(num_value_c0-1)
            prob_dict[f"C0_[{c1}]"] = [0] + [0]*(idx_of_1) + [1] + [0]*(num_value_c0-idx_of_1-2)
    ...
    for x in range(num_value_x):
        for c1 in range(num_value_c1):           
            for c0 in range(num_value_c0):           
                if c0==0:
                    prob_dict[f"Y_[{x} {c1} {c0}]"] = [0, 0, 0, 1] if x==2 else [1, 0, 0, 0]
                else:
                    prob_dict[f"Y_[{x} {c1} {c0}]"] = [0, 0, 1, 0] if x==3 else [0, 1, 0, 0]
