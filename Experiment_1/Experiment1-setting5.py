...
from stat_utils_5 import get_distrib_params, ent, ent_new, P_hat, E_hat_do_x_cA, E_true_do_x_cA
from graph import Node, Graph, CPD, CPDCollection

import pprint

from stat_utils_5 import get_V_index
...
# %%
from env_setting_5 import num_value_c1, num_value_c0, num_value_x, domain_y
...
# %%
from stat_utils_5 import get_parent_values_constrained
...
def sample_true_prob():
    ...
    for x in range(num_value_x):
        for c0 in range(num_value_c0):           
            if c0==0:
                prob_dict[f"Y_[{x} {c0}]"] = [0, 1, 0] if x==2 else [1, 0, 0]
            else:
...
for i in range(NUM_RUNS_PER_B*len(B_VALUES)):
    ...
    if MLFLOW_ENABLED:
        ...
        mlflow.log_artifact("env_setting_5.py")
