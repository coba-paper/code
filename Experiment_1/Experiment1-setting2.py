...
from stat_utils_2 import get_distrib_params, ent, ent_new, P_hat, E_hat_do_x_cA, E_true_do_x_cA
from graph import Node, Graph, CPD, CPDCollection

import pprint

from stat_utils_2 import get_V_index
...
# %%
from env_setting_2 import num_value_c1, num_value_c0, num_value_x, domain_y
...
# %%
from stat_utils_2 import get_parent_values_constrained
...
def sample_true_prob():
    ...
    prob_dict["C1_[]"] = [0.3/(num_value_c1-1)]*(num_value_c1-1) + [0.7]

    for c1 in range(num_value_c1):
        prob_dict[f"C0_[{c1}]"] = [1, 0] if c1<(num_value_c1-1) else [0, 1]
...
for i in range(NUM_RUNS_PER_B*len(B_VALUES)):
    ...
    if MLFLOW_ENABLED:
        ...
        mlflow.log_artifact("env_setting_2.py")
