...
def beta(x, cA, N):
  ...
  return N[get_N_index(ids=['X']+cpds.g.get_list_of_CA_ids(), values=[x]+cA)] / \
    (get_true_prob_joint(ids=cpds.g.get_list_of_CA_ids(), values=cA, prob_dict=prob_dict))
...
def sample_true_prob():
    ...
    for c1 in range(num_value_c1):
        if c1<cutoff:
            ...
            prob_dict[f"C0_[{c1}]"] = [0]*(num_value_c0-1) + [1]
    ...
    for x in range(num_value_x):
        for c0 in range(num_value_c0):           
            if c0==0:
                prob_dict[f"Y_[{x} {c0}]"] = [0, 0, 0, 1] if x==2 else [1, 0, 0, 0]
            else:
                prob_dict[f"Y_[{x} {c0}]"] = [0, 0, 1, 0] if x==3 else [0, 1, 0, 0]
